...
module.exports = class extends Event {
  ...
  async run(reaction, user) {
    ...
    const threshold = this.client.plugins.clownboard.getThreshold(
      message.guild
    );
    if (users.size >= threshold) {
      if (!this.client.plugins.clownboard.getMsg(message.id)) {
        ...
        this.client.plugins.clownboard.saveMsg(message.id, msg.id);
      } else {
        const embed = new EmbedBuilder()
          .setAuthor({
            name: message.author.tag,
