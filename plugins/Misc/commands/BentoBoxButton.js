...
module.exports = class extends Command {
  ...
  async execute(message) {
    ...
    const joinMessage = await message.channel.send({ files: [gifPath], components: [row] });

    const filter = (interaction) => interaction.customId === 'join_game' && !interaction.user.bot;
    const collector = joinMessage.createMessageComponentCollector({ filter, time: 30000, componentType: 'BUTTON' });

    collector.on("collect", async (interaction) => {
      players.add(new Player(interaction.user));
